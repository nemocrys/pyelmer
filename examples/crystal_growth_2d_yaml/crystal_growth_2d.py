...
###############
# elmer setup
sim = elmer.load_simulation("axi-symmetric_steady", None, "yaml")
...
#solver_output.data.update({"Gmsh Format", "True"})
...
bndry_bottom.fixed_heatflux = 5314
# Alternative:
# bndry_bottom.load_boundary("fixed_heatflux_bottom")

bndry_crys_melt = elmer.Boundary(sim, "crys_melt", [if_crystal_melt.ph_id])
bndry_crys_melt.load_boundary("fixed_heatflux_crys_melt")

bndry_surfaces = elmer.Boundary(
    sim, "surfaces", [bnd_crystal_out.ph_id, bnd_melt.ph_id, bnd_crucible_outside.ph_id]
)
bndry_surfaces.load_boundary("heat_transfer_radiation_idealized")
