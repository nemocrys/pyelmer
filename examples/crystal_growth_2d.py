...
crucin = factory.addRectangle(
    0, -melt_h - (cruc_h - cruc_hd), 0, melt_r, cruc_h - cruc_hd
)
crucout = factory.addRectangle(
    melt_r, -melt_h - (cruc_h - cruc_hd), 0, cruc_r - melt_r, cruc_h
)
...
# detect boundaries
crucbotin = get_boundaries_in_box(
    0, -(cruc_hd + melt_h), 0, cruc_r, -(cruc_hd + melt_h), 0, 2, crucin
)
crucbotout = get_boundaries_in_box(
    0, -(cruc_hd + melt_h), 0, cruc_r, -(cruc_hd + melt_h), 0, 2, crucout
)
crucside = get_boundaries_in_box(
    cruc_r, -(cruc_hd + melt_h), 0, cruc_r, -(cruc_hd + melt_h) + cruc_h, 0, 2, crucout
)
cructop = get_boundaries_in_box(
    melt_r,
    -(cruc_hd + melt_h) + cruc_h,
    0,
    cruc_r,
    -(cruc_hd + melt_h) + cruc_h,
    0,
...
crucin = get_boundaries_in_box(
    melt_r, 0, 0, melt_r, -(cruc_hd + melt_h) + cruc_h, 0, 2, crucout
)
...
ph_rad = add_physical_group(
    1, [crucside, cructop, crucin, melttop, cryssurf, crystop], "rad"
)
...
bndry_bottom.data.update({"Heat Flux BC": True, "Heat Flux": "real 5314"})

bndry_crysint = elmer.Boundary(sim, "crysint", [ph_crysint])
bndry_crysint.data.update({"Heat Flux BC": True, "Heat Flux": "real 21289"})
